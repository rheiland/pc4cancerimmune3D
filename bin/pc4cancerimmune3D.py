...
def write_config_file(name):
    ...
    # print("pc4cancerimmune3D.py: ------- sub.numx, sub.numy = ", sub.numx, sub.numy)


# callback from write_config_button
# def write_config_file_cb(b):
#     path_to_share = os.path.join('~', '.local','share','pc4cancerimmune3D')
...
def get_config_files():
    ...
    if nanoHUB_flag:
        full_path = os.path.expanduser("~/data/results/.submit_cache/pc4cancerimmune3D")  # does Windows like this?
    else:
        # local cache
        try:
            cachedir = os.environ['CACHEDIR']
            full_path = os.path.join(cachedir, "pc4cancerimmune3D")
        except:
...
def run_done_func(s, rdir):
    ...
    if nanoHUB_flag:
        ...
        os.system("submit  mail2self -s 'nanoHUB pc4cancerimmune3D' -t 'Your Run completed.'&")
...
#    animate_tab.gen_button.disabled = True
    ...
    if nanoHUB_flag:
        if remote_cb.value:
            s.run(run_name, "-v ncn-hub_M@brown -n 8 -w 1440 pc4cancerimmune3D-r7 config.xml")   # "-r7" suffix??
        else:
...
if xml_root.find('.//cell_definitions'):
    titles = ['About', 'Config Basics', 'Microenvironment', 'User Params', 'Cell Types', 'Out: Plots', 'Fury']
    tabs = widgets.Tab(children=[about_tab.tab, config_tab.tab, microenv_tab.tab, user_tab.tab, cell_types_tab.tab, sub.tab, fury_tab.tab],
                   _titles={i: t for i, t in enumerate(titles)},
                   layout=tab_layout)
else:
    titles = ['About', 'Config Basics', 'Microenvironment', 'User Params', 'Out: Plots', 'Fury']
    tabs = widgets.Tab(children=[about_tab.tab, config_tab.tab, microenv_tab.tab, user_tab.tab, sub.tab, fury_tab.tab],
                   _titles={i: t for i, t in enumerate(titles)},
                   layout=tab_layout)

homedir = os.getcwd()

tool_title = widgets.Label(r'\(\textbf{PhysiCell 3D tumor and Fury visualization}\)')
if nanoHUB_flag or hublib_flag:
